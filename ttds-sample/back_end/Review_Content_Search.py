...
class Review_Content_Searcher:
    ...
    def search(self, phrase, main_dictionary, Max_num=150):# Take a String like:"feel emotionally connected",
                                                            #  return search results as a dictionary
        TFIDF_results = TFIDF_search(phrase, words_dic = self.words_dic, max_result_num = Max_num)
        return self.transform_content_search_result_to_dictionary(TFIDF_results, main_dictionary)

    # 传入movie_name, year 和 review_id, 返回一个字典
    def get_review(self, movie_name, year, review_id, main_dictionary):
        for review in main_dictionary[movie_name][year][2]:
    ...
    def transform_content_search_result_to_dictionary(self, TFIDF_search_results, main_dictionary):
        returned_dic = {}
        index = 0
...
if __name__ =="__main__":
    ...
    result = review_Content_Searcher.search("feel emotionally connected", main_dic)
    print(result)